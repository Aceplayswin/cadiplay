...
exclusively on the **master database** (`cadiplay_master`) and provisions isolated
tenant databases for products like Cadiplay, Product B, etc.

This service is deployed separately from the per-product API (`cadiplay/api`) at
...
mysql -u root cadiplay_master < database/master.sql
```
