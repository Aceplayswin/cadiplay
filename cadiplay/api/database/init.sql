-- CADIPLAY Platform - MySQL Schema
...
-- the platform_settings['affiliate_program'] default".
--
-- Two different meanings that both got called "tier" during design, kept apart:
--   commission_tier -> NETWORK DEPTH  (1 = direct partner, 2 = their sub, ...)
--   tier_label      -> LOYALTY BAND   ('Bronze' / 'Gold' / 'Platinum')
CREATE TABLE IF NOT EXISTS affiliates (
  ...
  -- What this row's PARENT earns on this row's commission.
  override_rate DECIMAL(5,2) DEFAULT 0,
  parent_affiliate_id BIGINT UNSIGNED,
  total_referrals INT DEFAULT 0,
  total_commission DECIMAL(18,2) DEFAULT 0,
  payout_threshold DECIMAL(18,2) DEFAULT 0,
  -- Display caches, refreshed by the commission run and the payout transitions.
  pending_commission DECIMAL(18,2) DEFAULT 0,
  approved_commission DECIMAL(18,2) DEFAULT 0,
  paid_commission DECIMAL(18,2) DEFAULT 0,
  -- A losing day carries forward so tomorrow's revenue share nets it off first.
  ngr_carry_forward DECIMAL(18,2) DEFAULT 0,
  is_active BOOLEAN DEFAULT TRUE,
  ...
  INDEX idx_aff_parent (parent_affiliate_id),
  INDEX idx_aff_email (email)
  -- No FK on parent_affiliate_id: it does not prevent cycles, which is the
  -- failure that actually matters here. Parent validity, acyclicity and max
  -- depth are enforced in core/affiliate_services.py.
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Addressed to exactly one actor: a player, a staff member, or an affiliate.
-- All three columns are nullable and none carries an FK, which is what let the
-- affiliate program add a third recipient type without touching the other two.
CREATE TABLE IF NOT EXISTS notifications (
  ...
  INDEX idx_notif_affiliate (affiliate_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- ---------------------------------------------------------------------------
-- Affiliate program: tracking, attribution, commission ledger and payouts.
-- Everything below hangs off `affiliates` above. Shipped as migration
-- database/migrations/002_affiliate_program.sql for databases that predate it.
-- ---------------------------------------------------------------------------
...
VALUES (
  1, 'superadmin', 'admin@cadiplay.local',
...
-- from the production database (dump-cadiplay-202607181724.sql) so a fresh
...
-- ---------------------------------------------------------------------------
DROP PROCEDURE IF EXISTS _cadiplay_add_column;
DELIMITER $$
CREATE PROCEDURE _cadiplay_add_column(
...
CALL _cadiplay_add_column('bonuses', 'referrer_reward',     "referrer_reward DECIMAL(18,2) DEFAULT 0 AFTER max_bonus_cap");
CALL _cadiplay_add_column('bonuses', 'credit_target',       "credit_target ENUM('bonus','main') DEFAULT 'bonus' AFTER wagering_multiplier");
CALL _cadiplay_add_column('bonuses', 'promo_code',          "promo_code VARCHAR(40) AFTER claim_method");
CALL _cadiplay_add_column('bonuses', 'per_user_limit',      "per_user_limit INT AFTER promo_code");
CALL _cadiplay_add_column('bonuses', 'total_budget',        "total_budget DECIMAL(18,2) AFTER per_user_limit");
CALL _cadiplay_add_column('bonuses', 'total_awarded',       "total_awarded DECIMAL(18,2) DEFAULT 0 AFTER total_budget");
CALL _cadiplay_add_column('bonuses', 'total_claims',        "total_claims INT DEFAULT 0 AFTER total_awarded");
CALL _cadiplay_add_column('bonuses', 'bonus_validity_days', "bonus_validity_days INT DEFAULT 30 AFTER total_claims");
...
CALL _cadiplay_add_column('bonuses', 'is_first_deposit',    "is_first_deposit BOOLEAN DEFAULT FALSE AFTER min_deposit");
CALL _cadiplay_add_column('bonuses', 'is_second_deposit',   "is_second_deposit BOOLEAN DEFAULT FALSE AFTER is_first_deposit");
CALL _cadiplay_add_column('bonuses', 'is_third_deposit',    "is_third_deposit BOOLEAN DEFAULT FALSE AFTER is_second_deposit");
CALL _cadiplay_add_column('bonuses', 'is_new_player_only',  "is_new_player_only BOOLEAN DEFAULT FALSE AFTER is_third_deposit");
CALL _cadiplay_add_column('bonuses', 'new_player_days',     "new_player_days INT DEFAULT 7 AFTER is_new_player_only");
CALL _cadiplay_add_column('bonuses', 'scope',               "scope ENUM('mass','targeted') DEFAULT 'mass' AFTER claim_method");
CALL _cadiplay_add_column('bonuses', 'target_user_id',      "target_user_id BIGINT UNSIGNED AFTER scope");

-- user_bonuses: audit + duplicate-suppression columns.
CALL _cadiplay_add_column('user_bonuses', 'credit_target',  "credit_target ENUM('bonus','main') DEFAULT 'bonus' AFTER wagering_completed");
CALL _cadiplay_add_column('user_bonuses', 'source',         "source ENUM('joining','deposit','referral','game','cashback','promo','manual') NOT NULL DEFAULT 'manual' AFTER credit_target");
CALL _cadiplay_add_column('user_bonuses', 'transaction_id', "transaction_id BIGINT UNSIGNED AFTER source");
CALL _cadiplay_add_column('user_bonuses', 'granted_by',     "granted_by BIGINT UNSIGNED AFTER transaction_id");
CALL _cadiplay_add_column('user_bonuses', 'notes',          "notes VARCHAR(255) AFTER granted_by");
CALL _cadiplay_add_column('user_bonuses', 'updated_at',     "updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
...
CALL _cadiplay_add_column('user_bonuses', 'award_mode',     "award_mode ENUM('locked','pending') NOT NULL DEFAULT 'locked' AFTER credit_target");
CALL _cadiplay_add_column('user_bonuses', 'completed_at',   "completed_at DATETIME AFTER expires_at");
...
CALL _cadiplay_add_column('user_settings', 'referral_code', "referral_code VARCHAR(20) AFTER agent_id");
CALL _cadiplay_add_column('user_settings', 'referred_by',   "referred_by BIGINT UNSIGNED AFTER referral_code");
...
CALL _cadiplay_add_column('game_providers', 'agency_uid',         "agency_uid VARCHAR(100) AFTER is_active");
CALL _cadiplay_add_column('game_providers', 'aes_secret_key',     "aes_secret_key VARCHAR(128) AFTER agency_uid");
CALL _cadiplay_add_column('game_providers', 'server_url',         "server_url VARCHAR(255) AFTER aes_secret_key");
CALL _cadiplay_add_column('game_providers', 'launch_path',        "launch_path VARCHAR(100) AFTER server_url");
CALL _cadiplay_add_column('game_providers', 'player_prefix',      "player_prefix VARCHAR(40) AFTER launch_path");
CALL _cadiplay_add_column('game_providers', 'callback_path',      "callback_path VARCHAR(100) AFTER player_prefix");
CALL _cadiplay_add_column('game_providers', 'currency_code',      "currency_code VARCHAR(10) AFTER callback_path");
-- Sports/lottery vendors settle long after the stake; their rounds stay
-- Pending in bet history until the result callback lands.
CALL _cadiplay_add_column('game_providers', 'delayed_settlement', "delayed_settlement BOOLEAN NOT NULL DEFAULT FALSE AFTER currency_code");
...
CALL _cadiplay_add_column('game_rounds', 'game_name',    "game_name VARCHAR(150) AFTER game_uid");
CALL _cadiplay_add_column('game_rounds', 'settle_status', "settle_status ENUM('pending','settled') NOT NULL DEFAULT 'settled' AFTER game_round");
CALL _cadiplay_add_column('game_rounds', 'settled_at',   "settled_at DATETIME AFTER settle_status");


-- affiliates: the placeholder table gains a real identity (password, status),
-- its commission configuration, and the application fields the /apply form
-- captures. notifications gains a third recipient type.
CALL _cadiplay_add_column('affiliates', 'password_hash',       "password_hash VARCHAR(255) AFTER email");
CALL _cadiplay_add_column('affiliates', 'phone',               "phone VARCHAR(20) AFTER password_hash");
CALL _cadiplay_add_column('affiliates', 'company_name',        "company_name VARCHAR(150) AFTER phone");
CALL _cadiplay_add_column('affiliates', 'status',              "status ENUM('pending','info_requested','approved','rejected','suspended') DEFAULT 'pending' AFTER is_active");
CALL _cadiplay_add_column('affiliates', 'tier_label',          "tier_label VARCHAR(20) DEFAULT 'Bronze' AFTER commission_tier");
CALL _cadiplay_add_column('affiliates', 'commission_type',     "commission_type ENUM('revenue_share','cpa','hybrid') DEFAULT 'revenue_share' AFTER tier_label");
CALL _cadiplay_add_column('affiliates', 'commission_rate',     "commission_rate DECIMAL(5,2) DEFAULT 0 AFTER commission_type");
CALL _cadiplay_add_column('affiliates', 'cpa_amount',          "cpa_amount DECIMAL(18,2) DEFAULT 0 AFTER commission_rate");
CALL _cadiplay_add_column('affiliates', 'hybrid_cpa_days',     "hybrid_cpa_days INT DEFAULT 0 AFTER cpa_amount");
-- What this row's PARENT earns on this row's commission (not what this row earns).
CALL _cadiplay_add_column('affiliates', 'override_rate',       "override_rate DECIMAL(5,2) DEFAULT 0 AFTER hybrid_cpa_days");
CALL _cadiplay_add_column('affiliates', 'payout_threshold',    "payout_threshold DECIMAL(18,2) DEFAULT 0 AFTER total_commission");
-- Display caches, recomputed by the commission run and the payout transitions.
CALL _cadiplay_add_column('affiliates', 'pending_commission',  "pending_commission DECIMAL(18,2) DEFAULT 0 AFTER payout_threshold");
CALL _cadiplay_add_column('affiliates', 'approved_commission', "approved_commission DECIMAL(18,2) DEFAULT 0 AFTER pending_commission");
CALL _cadiplay_add_column('affiliates', 'paid_commission',     "paid_commission DECIMAL(18,2) DEFAULT 0 AFTER approved_commission");
-- A losing day carries forward so tomorrow's revenue share nets it off first.
CALL _cadiplay_add_column('affiliates', 'ngr_carry_forward',   "ngr_carry_forward DECIMAL(18,2) DEFAULT 0 AFTER paid_commission");
CALL _cadiplay_add_column('affiliates', 'two_factor_enabled',  "two_factor_enabled BOOLEAN DEFAULT FALSE");
CALL _cadiplay_add_column('affiliates', 'two_factor_secret',   "two_factor_secret VARCHAR(64)");
CALL _cadiplay_add_column('affiliates', 'two_factor_confirmed_at', "two_factor_confirmed_at DATETIME");
CALL _cadiplay_add_column('affiliates', 'kyc_status',          "kyc_status ENUM('none','pending','verified','rejected') DEFAULT 'none'");
...
CALL _cadiplay_add_column('affiliates', 'traffic_source',      "traffic_source VARCHAR(60)");
CALL _cadiplay_add_column('affiliates', 'expected_volume',     "expected_volume VARCHAR(40)");
CALL _cadiplay_add_column('affiliates', 'payment_preference',  "payment_preference VARCHAR(40)");
CALL _cadiplay_add_column('affiliates', 'application_notes',   "application_notes TEXT");
CALL _cadiplay_add_column('affiliates', 'rejection_reason',    "rejection_reason VARCHAR(500)");
CALL _cadiplay_add_column('affiliates', 'applied_at',          "applied_at DATETIME");
CALL _cadiplay_add_column('affiliates', 'approved_at',         "approved_at DATETIME");
CALL _cadiplay_add_column('affiliates', 'approved_by',         "approved_by BIGINT UNSIGNED");
CALL _cadiplay_add_column('affiliates', 'last_login_at',       "last_login_at DATETIME");
CALL _cadiplay_add_column('affiliates', 'updated_at',          "updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
CALL _cadiplay_add_column('notifications', 'affiliate_id', "affiliate_id BIGINT UNSIGNED AFTER admin_id");
...
CALL _cadiplay_add_column('agents', 'username',          "username VARCHAR(50) AFTER code");
CALL _cadiplay_add_column('agents', 'password_hash',     "password_hash VARCHAR(255) AFTER username");
CALL _cadiplay_add_column('agents', 'parent_agent_id',   "parent_agent_id BIGINT UNSIGNED AFTER name");
CALL _cadiplay_add_column('agents', 'level',             "level ENUM('super_admin','admin','super_master','master','agent') NOT NULL DEFAULT 'agent' AFTER parent_agent_id");
CALL _cadiplay_add_column('agents', 'depth',             "depth INT NOT NULL DEFAULT 1 AFTER level");
CALL _cadiplay_add_column('agents', 'tree_path',         "tree_path VARCHAR(255) AFTER depth");
CALL _cadiplay_add_column('agents', 'credit_reference',  "credit_reference DECIMAL(18,2) DEFAULT 0 AFTER commission_type");
CALL _cadiplay_add_column('agents', 'balance',           "balance DECIMAL(18,2) DEFAULT 0 AFTER credit_reference");
CALL _cadiplay_add_column('agents', 'exposure',          "exposure DECIMAL(18,2) DEFAULT 0 AFTER balance");
CALL _cadiplay_add_column('agents', 'partnership',       "partnership DECIMAL(5,2) DEFAULT 0 AFTER exposure");
CALL _cadiplay_add_column('agents', 'settled_pl',        "settled_pl DECIMAL(18,2) DEFAULT 0 AFTER total_commission");
CALL _cadiplay_add_column('agents', 'unsettled_pl',      "unsettled_pl DECIMAL(18,2) DEFAULT 0 AFTER settled_pl");
CALL _cadiplay_add_column('agents', 'status',            "status ENUM('active','suspended','locked','closed') NOT NULL DEFAULT 'active' AFTER is_active");
CALL _cadiplay_add_column('agents', 'bet_locked',        "bet_locked BOOLEAN DEFAULT FALSE AFTER status");
...
CALL _cadiplay_add_column('agents', 'last_login_ip',     "last_login_ip VARCHAR(45)");
CALL _cadiplay_add_column('agents', 'created_by',        "created_by BIGINT UNSIGNED");
CALL _cadiplay_add_column('agents', 'updated_at',        "updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
...
CALL _cadiplay_add_column('agents', 'company_name',          "company_name VARCHAR(150) AFTER name");
CALL _cadiplay_add_column('agents', 'market_region',         "market_region VARCHAR(80)");
CALL _cadiplay_add_column('agents', 'expected_volume',       "expected_volume VARCHAR(40)");
CALL _cadiplay_add_column('agents', 'experience',            "experience VARCHAR(40)");
CALL _cadiplay_add_column('agents', 'application_notes',     "application_notes TEXT");
CALL _cadiplay_add_column('agents', 'requested_parent_code', "requested_parent_code VARCHAR(20)");
CALL _cadiplay_add_column('agents', 'rejection_reason',      "rejection_reason VARCHAR(500)");
CALL _cadiplay_add_column('agents', 'applied_at',            "applied_at DATETIME");
CALL _cadiplay_add_column('agents', 'approved_at',           "approved_at DATETIME");
CALL _cadiplay_add_column('agents', 'approved_by',           "approved_by BIGINT UNSIGNED");

-- wallets: the sportsbook's own promotional bucket, alongside the casino one.
CALL _cadiplay_add_column('wallets', 'sports_bonus_balance',
...
DROP PROCEDURE IF EXISTS _cadiplay_add_index;
DELIMITER $$
CREATE PROCEDURE _cadiplay_add_index(
...
CALL _cadiplay_add_index('game_rounds', 'idx_gr_settle',    'settle_status');
CALL _cadiplay_add_index('game_rounds', 'idx_gr_round',     'user_id, game_round');
CALL _cadiplay_add_index('game_rounds', 'idx_gr_win',       'win_amount, created_at');

-- Affiliate lookups: the applications queue filters on status, the network tree
-- walks parents, and login resolves by email.
CALL _cadiplay_add_index('affiliates',    'idx_aff_status',       'status');
CALL _cadiplay_add_index('affiliates',    'idx_aff_parent',       'parent_affiliate_id');
CALL _cadiplay_add_index('affiliates',    'idx_aff_email',        'email');
CALL _cadiplay_add_index('notifications', 'idx_notif_affiliate',  'affiliate_id');
...
CALL _cadiplay_add_index('agents',        'idx_agents_parent',    'parent_agent_id');
CALL _cadiplay_add_index('agents',        'idx_agents_path',      'tree_path');
CALL _cadiplay_add_index('agents',        'idx_agents_status',    'status');
CALL _cadiplay_add_index('user_settings', 'idx_us_agent',         'agent_id');

-- The applications queue filters on who was named as upline, and the public
-- status check resolves by contact_email.
CALL _cadiplay_add_index('agents', 'idx_agents_email',            'contact_email');
CALL _cadiplay_add_index('agents', 'idx_agents_requested_parent', 'requested_parent_code');
