# Cadiplay Agent Panel — Developer Documentation
...
The **downline-facing console** for the Cadiplay platform: an agent signs in, sees the
...
reason `cadiplay/affiliate` is: agents are a distinct trust boundary. There are now four
portals:
...
| Affiliate | `cadiplay/affiliate` | 3003 | External marketing partners |
| **Agent** | `cadiplay/agent` | **3004** | Downline operators |
...
| Piece | File |
|---|---|
| Models | `cadiplay/api/core/agent_models.py` |
| JWT guard | `cadiplay/api/core/agent_auth.py` (`require_agent`) |
| Business logic | `cadiplay/api/core/agent_services.py` |
| HTTP layer | `cadiplay/api/core/agent_views.py` |
| Routes | `cadiplay/api/core/agent_urls.py` → mounted at `/api/v1/agent/…` |
| Schema | `cadiplay/api/database/init.sql` + `database/migrations/003_agent_panel.sql`, `004_agent_applications.sql` |
...
```bash
mysql -u root <tenant_db> < cadiplay/api/database/migrations/003_agent_panel.sql
mysql -u root <tenant_db> < cadiplay/api/database/migrations/004_agent_applications.sql
```
